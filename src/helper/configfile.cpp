...
namespace helper {
  ...
  {
    std::string tmp = line;
    if (tmp.find('=') == tmp.npos) return false; // not in key=value format
    tmp.erase(0, tmp.find_first_not_of("\t ")); // trim in front
    if (tmp[0] == '=') return false; // forgotten key
    return true;
  }
  ...
  void ConfigFile::trim(std::string &str) const
  {
    ...
    if (pos != str.npos) {
      ...
      if (pos != str.npos) str.erase(0, pos); // Remove everything in front
