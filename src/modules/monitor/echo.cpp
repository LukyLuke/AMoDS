...
namespace amods {
  namespace monitor {
    ...
    void Echo::SendEchoRequest(pingstat *res, unsigned int num, unsigned int timeout_ms) {
      ...
      // Fill the whole Datapasrt of the ICMP-Data with 'E' as Pseudo-Data
      char *datapart;
    ...
    void Echo::ParseResponse(struct pingstat *resp, char *received, int bytes_read, struct sockaddr_in *from) {
      ...
      // TODO: Why do we receive here our own ECHO-Ping Request when we ping 127.0.0.1
      //       tried it also with a toimeout of couple microseconds after sending but that did not resolve the problem
